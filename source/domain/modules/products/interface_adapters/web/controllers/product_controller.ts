import mapper from './mappers/product_mapper';
import service from 'domain/modules/products/app_business_rules';
...
export class ProductController {
    ...
    async createMany(req: any, res: Response, next: NextFunction) {
        try {// Funciona, pero se puede hacer mejor?
            const mapProducts = async (products: any[],Metodo:Function) => {
                const productsDOM: any[] = [];
                for (const product of products) {
                    const entity = Metodo(product)
                    productsDOM.push(entity);
                }
                return productsDOM;
            };
            const body = req.body;
            const productsDOM = await mapProducts(body,mapper.fromApiToDom);
            const products = await service.createMany(productsDOM)
            const productsAPI = await mapProducts(products,mapper.fromDomToApi);
            res.status(HTTPCodesEnum.CREATED).json(
                new ApiResponse(
                    HTTPCodesEnum.CREATED,
                    productsAPI
